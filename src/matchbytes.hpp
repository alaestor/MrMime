...
class MatchBytes
{
	...
	{
		for (const auto&& [b,c] : fgl::zip::zip(m_bytes, cstr))
	...
	std::byte operator[](const decltype(m_bytes)::size_type i) const
	{ return m_bytes.at(i); }

	[[nodiscard]] constexpr inline
	std::byte& operator[](const decltype(m_bytes)::size_type i)
	{ return m_bytes.at(i); }
